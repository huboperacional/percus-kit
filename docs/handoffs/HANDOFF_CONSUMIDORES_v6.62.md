...
| 6.58.0 | **Trilhos P / M / G (R9).** P = mexe no que existe, até ~5 arquivos; M = feature nova ou muda o que é gravado; G = schema, auth, pagamento, integração que envia/cobra/publica, segredo, tracking, hook/canon, deploy. | Declare o trilho e a estimativa na primeira resposta; nunca rebaixe. P/M: plano curto, uma revisão por lote, no máximo 2 rodadas de conserto, sem marco nem `✓`, sem conselho automático, R11 chamando o **wrapper** `scripts/percus-review-auto.ps1` com `-NoFactCheck` (`.sh`: `--no-fact-check`) — o cliente `deepseek-review.ps1` **não tem esse switch** e erra se receber. Só G tem marco, `milestone-review` e conselho automático. Uma frente por sessão. |

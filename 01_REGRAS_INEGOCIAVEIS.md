...
- **Ciclo de vida da sessão — `login nunca destrói sessão` (v6.30.8+, aprendizado 2026-07-23):** **login não derruba nenhuma sessão existente**, e **logout derruba só o serviço que pediu**. Nada no auth pode derrubar sessão **cross-produto**: a chave da família de refresh é o `sub` (`canal:destino`), ou seja **a pessoa** — invalidar por `sub` num login apaga as sessões daquela pessoa em **todos** os produtos. Foi exatamente esse o bug que fez nenhum produto Percus manter login por 1 dia: um login por magic-link em qualquer produto derrubava todos os outros, gerando loop de re-OTP cross-produto (custo direto por mensagem na Cloud API paga). Roubo de token é tratado por **rotation + reuse-detection**, que já é a defesa do RFC — não por invalidação em massa no login. **Não construa "logout-all"**: cross-produto reintroduz o bug sob demanda e vira vetor de DoS; "sair deste serviço" já é o revoke da família apresentada. Se a expectativa de "sair de todos os dispositivos" aparecer, a forma é uma tela de dispositivos conectados revogando **um a um**. Trave com barreira estática (AST) a chamada da primitiva de wipe-por-subject a partir de código de aplicação.
- **Consumidor é obrigado a guardar e rotacionar o refresh** — sem isso a sessão morre no TTL do access (minutos), por mais correto que o auth esteja. O consumidor **precisa**: ler o `rt` das **três** portas (JSON do validate, e o fragmento `#at=…&rt=…` do magic-link **e** do hop SSO); persistir **antes** de qualquer redirect ou `history.replaceState` (no cookie httpOnly+Secure+SameSite=lax do bullet abaixo — ou em `localStorage` **apenas** sob a exceção de SPA pura descrita ali); renovar de forma proativa e/ou no primeiro `401`; e **sobrescrever o `rt` guardado a cada rotação** (o refresh é single-use — reusar o antigo mata a família). Parser de fragmento que lê só `at=` é o defeito mais comum e é invisível: o login funciona, só não dura.
- **Cookie httpOnly + Secure + SameSite=lax** sempre. Domínio compartilhado entre subdomínios do apex. Cross-domain via redirect-fragment (R16). **`localStorage` pra token é vetado por padrão** (exposto a XSS). **Exceção documentada (2026-07-24):** SPA pura **sem backend próprio** (não tem onde cunhar cookie server-side) pode guardar o `rt` em `localStorage` como trade-off **explícito e registrado** — o risco (XSS exfiltra o `rt` de 30d) é mitigado por rotação single-use + reuse-detection (RFC 6749 §10.4), e o caminho para fechar a janela é um **BFF** (backend fininho segurando o cookie httpOnly + proxy do refresh same-origin, padrão `tiatendo`/`paid-media`). Consumer **com** backend **não** tem a exceção: usa httpOnly. Operam hoje nessa exceção, com aval do operador: `familia`, `micro-investors`, `social-midia`.

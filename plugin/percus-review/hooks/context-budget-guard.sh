...
# cabeca: primeiro timestamp -> idade do transcript. HORA DE PAREDE NAO E GATILHO (6.52.0; ver o .ps1):
# HORAS so existe pra dar DIAS (resume velho) e pro log -- nunca pra decisao, nunca pra mensagem.
...
ST_N=0; ST_D=0
if [ -f "$STATE" ]; then
  ST_N=$(jq -r '.nivel // 0' "$STATE" 2>/dev/null); ST_D=$(jq -r 'if .dias then 1 else 0 end' "$STATE" 2>/dev/null)
fi
FALA=0
...
ND=$(( COND_D | ${ST_D:-0} ))
jq -n --argjson n "$NN" --argjson d "$([ $ND -eq 1 ] && echo true || echo false)" --argjson t "$TOKENS" \
  '{nivel:$n,dias:$d,tokens:$t,ts:(now|todate)}' > "$STATE" 2>/dev/null
...
if [ "$JANELA_INCERTA" -eq 1 ]; then
  MSG="[percus:context-budget] contexto vivo ~${K}k tokens. Janela ${JANELA_FONTE} -- entao ela e MAIOR que isso, mas eu nao sei quanto, e sem o denominador nao da pra dizer se voce esta perto do teto. Defina PERCUS_CTX_WINDOW pra eu voltar a medir percentual."
else
  MSG="[percus:context-budget] contexto vivo ~${K}k tokens (${PCT}% de uma janela ~${KJ}k -- ${JANELA_FONTE}); limiar de aviso ${KW}k."
fi

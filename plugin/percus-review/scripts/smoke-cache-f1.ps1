...
$pass = ($c1.usage.cache_creation_input_tokens -ge 1024) -and
        ($c1.usage.cache_read_input_tokens -eq 0) -and
        ($c2.usage.cache_read_input_tokens -ge 1024) -and
        ($c2.usage.cache_creation_input_tokens -eq 0)

if ($pass) {
    Write-Host "Cache F.1 ATIVO" -ForegroundColor Green
    exit 0
} else {
    Write-Host "Cache nao ativou conforme esperado - investigar" -ForegroundColor Red
    Write-Host "Verifique: 1) system-prompt-consult.md tem >= 1024 tok efetivos? 2) wrapper realmente le o arquivo?"
    exit 1
}

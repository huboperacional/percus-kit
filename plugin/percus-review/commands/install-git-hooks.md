...
**Onde mora a autorização (worktree):** o hook resolve a raiz pelo **repositório publicado** (`git rev-parse --path-format=absolute --git-common-dir` → pasta pai; requer **git >= 2.31**, senão fail-closed), não pela work tree nem pelo cwd — assim `GIT_WORK_TREE`/`--work-tree`, `core.worktree`, `GIT_COMMON_DIR` e um JSON forjado noutra pasta não trocam o repositório lido; `GIT_DIR`/`--git-dir` definem o repositório empurrado, e a autorização usada é sempre a desse repositório. Para um push disparado de um **worktree**, isso cai no **checkout principal**: no kit, a autorização mora SEMPRE em `D:\Claud Automations\percus-kit\.percus\`, inclusive para push de worktree. Como a sessão do agente roda a partir do **checkout principal**, a camada 1 (`external-action-guard`, que lê o cwd da sessão) lê esse MESMO `.percus\` — as duas camadas concordam nesse caso. Se a sessão rodasse de dentro do próprio worktree, a camada 1 leria o `.percus\` do worktree (que não existe) e a 2 seguiria no principal: alinhar a camada 1 à mesma regra (`--git-common-dir` → pai) fica como requisito. **Submódulo com o hook instalado, repositório bare e repositório criado com `--separate-git-dir` (git dir fora de `*/.git`) não são suportados:** o hook bloqueia sempre (fail-closed, lado seguro), coerente com o modelo de ameaça.

**Qual repositório manda (rodada 3):** o hook usa o `GIT_DIR` que o git lhe entrega — sempre o git dir do repositório **empurrado** — e NÃO o cwd. Com `--git-dir`/`GIT_DIR` explícito o hook roda no cwd de quem chamou; por isso `cd <repo com autorização> && git --git-dir=<outro>/.git push` **bloqueia** (autorização em A nunca publica B), e o uso legítimo `git --git-dir=<T>/.git --work-tree=<T> push` rodado de qualquer pasta **passa** com a autorização de `<T>` e audita em `<T>`. `GIT_WORK_TREE`/`--work-tree`/`core.worktree`/`GIT_COMMON_DIR` não trocam o repositório lido. Um `GIT_DIR` apontando outro repositório com hook e autorização **próprios** publica aquele repositório com a autorização dele — classe declarada "cópia/outro `.git`".

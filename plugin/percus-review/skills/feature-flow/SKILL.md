---
name: feature-flow
description: Use when starting any feature or bugfix in a Percus project. Declares the trilho P/M/G first (R9), then orchestrates R1->R13 workflow proportional to it (brainstorming -> plan -> execution -> TDD -> /percus-review:review -> mark [5-T]). Replaces loading R1+R9+R11+R13 separately.
---
...
trilho**. Sessão de consulta (ler código, explicar) não é feature e não declara trilho. Typo, texto de doc ou
rename sem mudança de comportamento ficam fora de trilho: só R11.

**Precedência (trilhos P e M):** a R9 e o `CLAUDE.md` do projeto têm precedência sobre `superpowers:brainstorming`,
`superpowers:writing-plans` e `superpowers:subagent-driven-development` — vale o que está abaixo, não o
default dessas skills (revisão por tarefa, 5 rodadas, revisão final no modelo mais capaz, ratchet "na dúvida,
o mais pesado"). Depois de 2 rodadas de conserto com achado crítico/importante aberto: **pare e promova o
trilho; nunca commite.**

## Fluxo (passos numerados)

### 1. Brainstorming pelo caminho do trilho (R9)
`superpowers:brainstorming`: P → caminho **Bounded**; M → **Bounded** por precedência do canon (design curto
no chat + aprovação, sem spec file, mesmo sem fluxo existente); G → **Architectural**. Spike só para pergunta
de viabilidade, em qualquer trilho.
...
- **P:** lista de tarefas em formato de contrato, até 1 página (no chat ou no PLANO).
- **M:** **plano de contrato** (formato na R9), até ~500 linhas.
...
| Trilho P | inline ou **um** implementador para a lista inteira; sem revisão por tarefa, só a final |
| Trilho M | **lotes de 2–4 tarefas** por implementador, uma revisão por lote, um commit por lote (R11 no diff do lote), até 2 rodadas, revisão final sonnet |
| Trilho G | `superpowers:subagent-driven-development`, uma tarefa por implementador |

Todo despacho de subagente (implementador ou revisor) usa `${env:PERCUS_CANON_DIR}/templates/DESPACHO_SUBAGENTE.template.md`.
...
(ou `.sh` em Unix). **Trilhos P e M:** acrescente `-NoFactCheck` (`.sh`: `--no-fact-check`) — fact-check F3 só no trilho G. Wrapper resolve plugin instalado + decide router + dispatch DeepSeek. Se output stderr tiver marker `__PERCUS_NEEDS_CROSS_CLAUDE__`, dispatch Sonnet subagent IMEDIATAMENTE via Agent tool com prompt R11 cross-claude-review e registre os findings com `registrar-review` (a linha pronta já vem no próprio marcador; grave o arquivo de findings fora do repo, `-Canal cross-claude`/`--canal cross-claude`) para liberar o commit pelo hash do diff.
...
- **P:** não há marco nem `✓`; a feature fecha no `[5-T]` com a R11 do diff final.
- **M:** não há `✓`; a revisão final do branch (sonnet) fecha a entrega no `[5-T]`, com R11.
...
- ✓ = milestone-review aprovado (só trilho G)
- 🎨 = draft de design aprovado (R10)

## Gate R1 — ciclo CRUD (ou forma visual) obrigatorio antes de [5-T]

Mudanca que nao cria, edita nem apaga dado usa a **forma visual** da R1: pergunte ao operador

```
[R1 gate - forma visual] Antes de marcar [5-T] na feature <X>: voce conferiu a tela real depois de F5? (sim/nao)
```

e use o trailer `UI-verified: YYYY-MM-DD HH:MM`. Qualquer caminho que grava dado segue o ciclo CRUD abaixo.

Ao tentar marcar uma feature que grava dado como `[5-T]` (testado), **exija confirmacao explicita do operador das 3 acoes CRUD com F5**:
...
2. **Inclua o trailer `CRUD-verified: YYYY-MM-DD HH:MM`** (ou `UI-verified: YYYY-MM-DD HH:MM` na forma visual) no commit que marca `[5-T]`. Use a data/hora reais da confirmacao. Exemplo:

...
# UMA nocao de "valor", usada pelas quatro regras: citado (aspas duplas ou simples, podendo conter
# espaco) OU nu. Duas rodadas de R11 foram gastas remendando aspa caso a caso; eram tres sintomas do
# mesmo buraco -- nao havia definicao de valor, havia tres aproximacoes.
$VALOR = '(?:"[^"]*"|''[^'']*''|[^\s"'']+)'
...
# 3. flag separada por ESPACO (--token VALOR) ou COLADA (-uusuario:senha).
...
# 4. header de credencial -- esquema ENUMERADO, nao opcional-livre.
